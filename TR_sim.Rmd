...
subj_n = 20    # number of subjects per language
lang_n = 2     # number of languages
...
## !! currently takes way too long (>10mins) on my machine to run, probably due to priors

priors <- c(prior(normal(0,1), class = Intercept),
            # weakly informative prior on SD and factor
...
test <- brm(resp_beta ~ diff_dur * as.factor(tonal.T) + (0 + diff_dur | subject),
                  data = df, 
             prior = priors,
...
pp_check(test)
summary(test) # not sure why the CrIs are so large, ESS are also a bit low
